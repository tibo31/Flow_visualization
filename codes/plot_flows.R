plot_flows <- function(y, index_o, index_d, type_plot = "flow_map", 
  ...
  if (type_plot == "Sankey") {
    ...
    if (ordering %in% c("descending", "clustering")) {
      special_levels_1 <- c(order_D[!order_D %in% order_O], order_O)
      data_long_2$names_flow <- paste0(data_long_2$names_flow, " ")
      boolScale <- scale_fill_manual(name = "Zone", values = rev(q4[special_levels_1]))
    } else {
      special_levels_1 <- (order_S)
      boolScale <- scale_fill_manual(name = "Zone", values = (q4[order_S]))
